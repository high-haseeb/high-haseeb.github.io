class NodeBase {
	...
	applyListeners() {
		...
		this.element.onmousemove = () => {
			if (!this.data.rippleOnMove) return;
	...
	updateNodeHelias() {
		...
		this.nextForce =
			(nodeUpForce + nodeDownForce + nodeRightForce + nodeLeftForce) / 2 -
			this.nextForce;
			...
			for (let xChange = -1; xChange <= 1; ++xChange) {
				...
				if (node.omniForce != 0) {
					this.updateForces(
						xChange,
						yChange,
						-xChange * node.omniForce,
						-yChange * node.omniForce,
					);
				} else {
					let deltaFx = 0,
						deltaFy = 0;
...
class AsciiNode extends NodeBase {
	constructor(xx, yy, data, mathMode) {
		...
		if (this.mathMode === "helias") this.drawNode = this.drawNodeHelias;
		else this.drawNode = this.drawNodeAnair;
	}
	getNodeElement() {
...
export class Puddle {
	...
	setNodeStyle(nodeStyle) {
		if (nodeStyle === "water") this.nodeStyle = WaterNode;
		else if (nodeStyle === "party") this.nodeStyle = PartyNode;
		else if (nodeStyle === "ascii") this.nodeStyle = AsciiNode;
		else if (nodeStyle === "base") this.nodeStyle = NodeBase;
		else console.log("Invalid nodeStyle value");
		this.setupGrid();
	}
	getNodeSize() {
	...
	setupGrid() {
		...
		this.parentNode.style.gridTemplateColumns =
			"repeat(" + this.numCols + ", " + this.nodeSize + "px)";
		this.parentNode.style.gridTemplateRows =
			"repeat(" + this.numRows + ", " + this.nodeSize + "px)";
	...
	createRandomRipple() {
		let rippleStrength = Math.floor(
			Math.random() * this.data.maxRippleStrength,
		);
	...
	createTimedRandomRipple() {
		...
		let timeoutChange =
			Math.floor(Math.random() * this.randomTimeRange) -
