...
float cnoise(vec3 P){
  ...
  vec3 color1 = vec3(0.0, 6.0 / 255.0, 220.0 / 255.0); // "0006DC" converted to vec3
  vec3 color2 = vec3(111.0 / 255.0, 239.0 / 255.0, 187.0 / 255.0); // "6FEFB" converted to vec3
...
float cnoise(vec3 P){
  ...
  void main() {
    ...
    gl_FragColor = vec4( mix(color1, color2, smoothstep(0.0, 1.0, normalize(cnoise(vPosition + time)) )  ) , 1.0);
  }
`;
  const fragmentMain = `
  // void main() {
    // gl_FragColor = vec4( mix(color1, color2, smoothstep(0.0, 1.0, normalize(cnoise(vPosition + time)) )  ) , 1.0);
vec4 diffuseColor =  vec4( mix(color1, color2, smoothstep(0.0, 1.0, normalize(cnoise(color1 + time)) )  ) , 1.0);
  `;
